...
class HbBookingForm {
	...
	private function load_scripts( $page_accom_id = 0, $atts = false ) {
		...
		if ( ! $script_loaded ) {
			...
			wp_enqueue_script( 'hb-front-end-script', plugin_dir_url( __FILE__ ) . 'js/booking-form.js', array( 'jquery' ), $this->utils->get_file_version( 'booking-form-js' ), true );
		}
	...
	public function hb_booking_form( $atts ) {
		...
		$this->load_scripts( $page_accom_id, $atts );
