...
function hb_rates( $atts, $plugin_version, $hbdb, $utils ) {
	...
	if ( $atts['seasons'] != '' ) {
		...
		foreach ( $season_names as $season_name ) {
			$season = $hbdb->get_season_by_name( trim( $season_name ) );
			if ( ! $season ) {
				return 'Invalid shortcode. The season "' . $season_name . '" does not exist.';
			}
			$season_dates = $hbdb->get_all_season_dates( $season['id'] );
			if ( $season_dates ) {
				usort( $season_dates, 'hb_compare_season_dates' );
				$season['dates'] = $season_dates;
				$seasons[] = $season;
				$all_seasons_dates = array_merge( $all_seasons_dates, $season_dates );
			}
		}
	} else if ( $atts['chronological'] == 'yes' ) {
		$seasons = array();
		$all_seasons_dates = $hbdb->get_all( 'seasons_dates' );
		usort( $all_seasons_dates, 'hb_compare_season_dates' );
		foreach ( $all_seasons_dates as $season_dates ) {
			$season = $hbdb->get_single( 'seasons', $season_dates['season_id'] );
		...
		usort( $seasons, 'hb_compare_season_dates' );
	}
	...
	if ( ( $atts['show_season_name'] == 'yes' ) && $price_per_night ) {
		$width = '20%';
	} else if ( ( $atts['show_season_name'] == 'yes' ) || $price_per_night ) {
		$width = '25%';
	} else {
		$width = '33%';
	}
	...
	$output = '
		...
		if ( $atts['show_season_name'] == 'yes' ) {
			$output .= '
					<th width="' . $width . '">' . $hbdb->get_string( 'table_rates_season' ) . '</th>';
		}
		$output .= '
	...
	foreach ( $seasons as $season ) {
		...
		foreach ( $output_dates as $j => $dates ) {
			...
			if ( ( $j == 0 ) && ( $atts['show_season_name'] == 'yes' ) ) {
				$season_name = $hbdb->get_string( 'season_' . $season['id'] );
				if ( ! $season_name ) {
...
function hb_compare_season_dates( $season_dates_a, $season_dates_b ) {
