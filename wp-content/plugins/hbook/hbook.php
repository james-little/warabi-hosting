...
require realpath(__DIR__) . '/hbook_bootstrap.php';
...
class HBook {
	...
	public function __construct() {
		...
		require HBOOK_ROOT_DIR . 'database-actions/database-actions.php';
		$this->hbdb = new HbDataBaseActions();

		require HBOOK_ROOT_DIR . 'accom-post-type/accom-post-type.php';
		$this->accommodation = new HbAccommodation( $this->hbdb );

		require HBOOK_ROOT_DIR . 'utils/utils.php';
		$this->utils = new HbUtils( $this->hbdb, $this->version );

		require HBOOK_ROOT_DIR . 'payment/payment-gateway.php';
		require HBOOK_ROOT_DIR . 'payment/paypal/paypal.php';
		require HBOOK_ROOT_DIR . 'payment/stripe/stripe.php';
		new HbPayPal( $this->hbdb, $this->version, $this->utils );
		$this->stripe = new HbStripe( $this->hbdb, $this->version );

		require HBOOK_ROOT_DIR . 'utils/options-utils.php';
		$this->options_utils = new HbOptionsUtils( $this->utils );

		require HBOOK_ROOT_DIR . 'utils/resa-ical.php';
		$this->resa_ical = new HbResaIcal( $this->hbdb, $this->utils );

		require HBOOK_ROOT_DIR . 'utils/plugin-check.php';
		$plugin_check = new HbPluginCheck( $this->version );

		require HBOOK_ROOT_DIR . 'admin-pages/admin-ajax-actions.php';
		$this->admin_ajax_actions = new HbAdminAjaxActions( $this->hbdb, $this->utils, $this->options_utils, $this->stripe );

		require HBOOK_ROOT_DIR . 'front-end/front-end-ajax-actions.php';
	...
	public function install_plugin() {
		...
		require HBOOK_ROOT_DIR . 'database-actions/database-creation.php';
		require HBOOK_ROOT_DIR . 'database-actions/database-schema.php';
	...
	public function plugin_activated() {
		...
		wp_schedule_event( time(), apply_filters( 'hb_ical_scheduled_event_frequency', 'hourly' ), 'hb_ical_synchronized' );
	...
	public function enqueue_wp_admin_style() {
        wp_enqueue_script( 'hb-global-js', HBOOK_ROOT_DIR . 'admin-pages/js/hb-global.js', array( 'jquery' ), $this->version );

		global $post_type;
		if ( $post_type == 'hb_accommodation' ) {
			wp_enqueue_style( 'hb-accom-style', HBOOK_ROOT_DIR . 'accom-post-type/accom-post-type.css', array(), $this->version );
			wp_enqueue_script( 'hb-accom-script', HBOOK_ROOT_DIR . 'accom-post-type/accom-post-type.js', array( 'jquery' ), $this->version );
	...
	public function enqueue_scripts( $hook ) {
		wp_enqueue_style( 'hb-admin-pages-style', HBOOK_ROOT_DIR . 'admin-pages/css/hb-admin-pages-style.css', array(), $this->version );
        wp_enqueue_script( 'hb-settings', HBOOK_ROOT_DIR . 'admin-pages/js/hb-settings.js', array( 'jquery' ), $this->utils->get_file_version( 'hb-settings-js' ) );
        ...
        if ( in_array( $page_name, $knockout_pages ) ) {
            wp_enqueue_script( 'hb-knockout', HBOOK_ROOT_DIR . 'admin-pages/js/knockout-3.2.0.js', array( 'jquery' ), $this->version );
            wp_enqueue_script( 'hb-settings-knockout', HBOOK_ROOT_DIR . 'admin-pages/js/hb-settings-knockout.js', array( 'jquery' ), $this->version );
        }
        ...
        if ( in_array( $page_name, $static_settings_pages ) ) {
            wp_enqueue_script( 'hb-settings-static', HBOOK_ROOT_DIR . 'admin-pages/js/hb-settings-static.js', array( 'jquery' ), $this->utils->get_file_version( 'hb-settings-static-js' ) );
        }

		if ( $page_name == 'fees' || $page_name == 'options' || $page_name == 'rates' ) {
			wp_enqueue_script( 'hb-options-and-fees-script', HBOOK_ROOT_DIR . 'admin-pages/js/hb-options-and-fees.js', array( 'jquery' ), $this->version, true );
        }

        if ( $page_name == 'fees' || $page_name == 'options' || $page_name == 'rates' || $page_name == 'reservations' ) {
			wp_enqueue_script( 'hb-price-utils', HBOOK_ROOT_DIR . 'admin-pages/js/hb-price-utils.js', array( 'jquery' ), $this->version, true );
		...
		if ( $page_name == 'reservations' ) {
			...
			wp_enqueue_script( 'hb-resa-utils', HBOOK_ROOT_DIR . 'admin-pages/pages/reservations/resa-utils.js', array( 'jquery' ), $this->version );
			wp_enqueue_script( 'hb-resa-cal', HBOOK_ROOT_DIR . 'admin-pages/pages/reservations/resa-cal.js', array( 'jquery' ), $this->utils->get_file_version( 'resa-cal-js' ) );
			wp_enqueue_script( 'hb-resa-export', HBOOK_ROOT_DIR . 'admin-pages/pages/reservations/resa-export.js', array( 'jquery' ), $this->version );
			wp_enqueue_style( 'hb-resa-cal-style', HBOOK_ROOT_DIR . 'admin-pages/pages/reservations/resa-cal.css', array(), $this->version );
		}

		if ( $page_name == 'forms' ) {
			wp_enqueue_script( 'jquery-ui-sortable' );
			wp_enqueue_script( 'hb-knockout-sortable', HBOOK_ROOT_DIR . 'admin-pages/js/knockout-sortable.min.js', array( 'jquery' ), $this->version );
			wp_enqueue_script( 'hb-form-builder', HBOOK_ROOT_DIR . 'admin-pages/js/hb-forms-and-contact.js', array( 'jquery' ), $this->version );
			wp_enqueue_style( 'hb-form-builder-style', HBOOK_ROOT_DIR . 'admin-pages/css/hb-form-builder.css', array(), $this->version );
		}
		...
		wp_enqueue_style( 'hb-' . $page_name . '-style', HBOOK_ROOT_DIR . 'admin-pages/pages/' . $page_name . '/' . $page_name . '.css', array(), $this->utils->get_file_version( $page_name . '-css' ) );
        wp_enqueue_script( 'hb-' . $page_name . '-script', HBOOK_ROOT_DIR . 'admin-pages/pages/' . $page_name . '/' . $page_name . '.js', array( 'jquery' ), $this->utils->get_file_version( $page_name . '-js' ), true );
	...
	public function display_admin_page() {
		...
		if ( current_user_can( 'manage_options' ) || ( $page_id == 'reservations' && current_user_can( 'read_resa' ) ) ) {
			require HBOOK_ROOT_DIR . 'admin-pages/admin-page.php';
			require HBOOK_ROOT_DIR . 'admin-pages/pages/' . $page_id . '/' . $page_id . '.php';
			$page_class = 'HbAdminPage' . ucfirst( $page_id );
			$admin_page = new $page_class( $page_id, $this->hbdb, $this->utils, new HbOptionsUtils( $this->utils ) );
			$admin_page->display();
		}
	}
	...
	public function hb_shortcodes( $atts, $content = '', $shortcode_name ) {
		...
		require HBOOK_ROOT_DIR . 'front-end/shortcode_' . $shortcode_name . '.php';

		switch ( $shortcode_name ) {

			case 'hb_availability' :
                wp_enqueue_script( 'hb-front-end-utils-script', HBOOK_ROOT_DIR . 'front-end/js/utils.js', array( 'jquery' ), $this->version, true );
			...
			case 'hb_booking_form' :
                wp_enqueue_script( 'hb-front-end-utils-script', HBOOK_ROOT_DIR . 'front-end/js/utils.js', array( 'jquery' ), $this->version, true );
				require HBOOK_ROOT_DIR . 'front-end/booking-form/search-form.php';
				require HBOOK_ROOT_DIR . 'front-end/form-fields.php';
                require HBOOK_ROOT_DIR . 'utils/resa-fees.php';
                require HBOOK_ROOT_DIR . 'front-end/booking-form/details-form.php';
